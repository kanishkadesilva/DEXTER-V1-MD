...
zokou({nomCom : "igdl" , categorie : "Download"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by *Qᴜᴇᴇɴ ᴛʜᴀʀᴜ-ᴍᴅ*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        zk.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "ig image downloader powered by *Qᴜᴇᴇɴ ᴛʜᴀʀᴜ-ᴍᴅ*"})
    }
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       ...
       zk.sendMessage(dest, { video: { url: result.hd  }, caption: 'facebook video downloader powered by *Qᴜᴇᴇɴ ᴛʜᴀʀᴜ-ᴍᴅ*' }, { quoted: ms });
